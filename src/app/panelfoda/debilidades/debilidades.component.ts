...
export class DebilidadesComponent implements OnInit {
	...
	@ViewChild('debilidades') dataTable: DataTableComponent;
	...
	columns: Array<any> = [
		...
		{title: 'Debilidad', name: 'foda_desc', columnName: 'foda_desc',
			filtering: {filterString: '', placeholder: 'Debilidad'}},
	...
	ngOnInit() {
		this.cargando = true;
		this.derechos = this._sidebarService.getDerechos('debilidades');
		...
		this._fodaService.getFODA(this.cuestion)
			.subscribe(
				data => {
					...
					this.dataTable.ruta_add = ['/panelfoda', 'debilidades_form', 'I', 0, this.cuestion];
