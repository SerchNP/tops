...
export class FortalezasComponent implements OnInit {
	...
	@ViewChild('fortalezas') dataTable: DataTableComponent;
	...
	columns: Array<any> = [
		...
		{title: 'Fortaleza', name: 'foda_desc', columnName: 'foda_desc',
			filtering: {filterString: '', placeholder: 'Fortaleza'}},
	...
	ngOnInit() {
		this.cargando = true;
		this.derechos = this._sidebarService.getDerechos('fortalezas');
		...
		this._fodaService.getFODA(this.cuestion)
			.subscribe(
				data => {
					...
					this.dataTable.ruta_add = ['/panelfoda', 'fortalezas_form', 'I', 0, this.cuestion];
