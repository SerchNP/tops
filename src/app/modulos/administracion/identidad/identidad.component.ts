...
export class IdentidadComponent implements OnInit, OnDestroy {
	...
	constructor(private activatedRoute: ActivatedRoute, private router: Router,
		...
		this.subscription = this.activatedRoute.url.subscribe(url => {
			...
			if (this.tipo === 'E') {
				this.ruta_add = ['/administracion', 'ejes_form', this.tipo, 'I', 0];
			} else {
				this.ruta_add = ['/administracion', 'submenuident', 'identidad_form', this.tipo, 'I', 0];
			}
		});
	}
	...
	editarIdentidad(identidad: any) {
		if (identidad.activo === 'N') {
			...
			if (this.tipo === 'E') {
				this.router.navigate(['/administracion', 'ejes_form', this.tipo, 'U', identidad.clave]);
			} else {
				this.router.navigate(['/administracion', 'submenuident', 'identidad_form', this.tipo, 'U', identidad.clave]);
			}
		}
	}
